...
library StringFormatting {
    ...
    uint256 constant DECIMALS6 = 6;
    uint256 constant ONE6 = 10 ** DECIMALS6;

    uint256 constant DECIMALS8 = 8;
    uint256 constant ONE8 = 10 ** DECIMALS8;

    uint256 constant DECIMALS2 = 2;
    uint256 constant ONE2 = 10 ** DECIMALS2;
    ...
    function decimal(uint256 n) internal pure returns (string memory) {
        ...
        if (fractionalPart == 0) {
            return string.concat(integerPart.groupRight());
        } else {
    ...
    function decimal6(uint256 n) internal pure returns (string memory) {
        if (n == type(uint256).max) {
            return "type(uint256).max";
        }

        uint256 integerPart = n / ONE6;
        uint256 fractionalPart = n % ONE6;
        ...
        if (fractionalPart == 0) {
            ...
            return string.concat(
                integerPart.groupRight(),
                DECIMAL_SEPARATOR,
                (ONE6 + fractionalPart).toString().slice(1).trimEnd("0")
            );
        }
    }

    function decimal8(uint256 n) internal pure returns (string memory) {
        if (n == type(uint256).max) {
            return "type(uint256).max";
        }

        uint256 integerPart = n / ONE8;
        uint256 fractionalPart = n % ONE8;
        ...
        if (fractionalPart == 0) {
            ...
            return string.concat(
                integerPart.groupRight(),
                DECIMAL_SEPARATOR,
                (ONE8 + fractionalPart).toString().slice(1).trimEnd("0")
            );
        }
    }

    function percent2(uint256 n) internal pure returns (string memory) {
        if (n == type(uint256).max) {
            return "type(uint256).max";
        }

        uint256 integerPart = n / ONE2;
        uint256 fractionalPart = n % ONE2;
        ...
        if (fractionalPart == 0) {
            ...
            return string.concat(
                integerPart.groupRight(),
                DECIMAL_SEPARATOR,
                (ONE2 + fractionalPart).toString().slice(1).trimEnd("0"),
                " %"
            );
        }
    }
